...
{
	DHT11_GPIO_Config();
	GPIO_SetBits(DHT11_Dout_GPIO_PORT,DHT11_Dout_GPIO_PIN);               // 拉高GPIOB10
}
...
//		if(GPIO_ReadInputDataBit ( DHT11_Dout_GPIO_PORT, DHT11_Dout_GPIO_PIN )==Bit_SET)/* x us后仍为高电平表示数据“1” */
//		{
//			/* 等待数据1的高电平结束 */
...
static uint8_t DHT11_ReadByte ( void )
{
	uint8_t i, temp=0;	
		for( i = 0; i < 8; i++) 
		{
			...
			//等待起始低电平结束
			while(GPIO_ReadInputDataBit(DHT11_Dout_GPIO_PORT, DHT11_Dout_GPIO_PIN) == Bit_RESET);
			//在26-28us(0)或70us(1)采样
		/*DHT11 以26~28us的高电平表示“0”，以70us高电平表示“1”，
		 *通过检测 x us后的电平即可区别这两个状 ，x 即下面的延时 
		 */
			Delay_us(40);  //采样点位于高电平中间  
			//动态组装字节(从高位到低位)
			temp <<= 1;  // 左移腾出最低位
				if(GPIO_ReadInputDataBit(DHT11_Dout_GPIO_PORT, DHT11_Dout_GPIO_PIN) == Bit_SET) 
				{
				 temp |= 0x01;  // 设置最低位
				 while(GPIO_ReadInputDataBit(DHT11_Dout_GPIO_PORT, DHT11_Dout_GPIO_PIN) == Bit_SET);
				}
...
{  
	...
	if(GPIO_ReadInputDataBit ( DHT11_Dout_GPIO_PORT, DHT11_Dout_GPIO_PIN )==Bit_RESET)     
	{
